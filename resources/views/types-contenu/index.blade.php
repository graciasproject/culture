    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div>
                    <nav class="flex mb-2" aria-label="Breadcrumb">
                        <ol class="flex items-center space-x-2 text-sm text-gray-500">
                            ...
                            <li>/</li>
                            <li class="font-medium text-gray-800 dark:text-gray-200">Types de Contenu</li>
                        </ol>
                    </nav>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
                        Types de <span class="text-benin-green">Contenu</span>
                    </h1>
                </div>

                <a href="{{ route('types-contenu.create') }}"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-700"
                            ...
                            @forelse($typesContenu as $type)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $type->contenus_count > 0 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600' }}">
                                            {{ $type->contenus_count }} contenus
                                        </span>
                                    </td>

                                    <td class="px-6 py-4 text-right space-x-2">
                                        <a href="{{ route('types-contenu.edit', $type->id) }}"
                                        ...
                                        <form action="{{ route('types-contenu.destroy', $type->id) }}" method="POST"
                                            class="inline-block"
                                            onsubmit="return confirm('Attention : Cette suppression est irréversible.');">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                class="inline-flex p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors {{ $type->contenus_count > 0 ? 'opacity-50 cursor-not-allowed' : '' }}"
                                                {{ $type->contenus_count > 0 ? 'disabled' : '' }} title="Supprimer">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-gray-500">
                                        <p class="text-lg">Aucun type de contenu défini.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                @if ($typesContenu instanceof \Illuminate\Pagination\LengthAwarePaginator)
